...
export const Route = createFileRoute("/revendedores")({
  head: () => ({
    meta: [
      { title: "Seja Revendedor — Chopp Artesanal Premium | Golden Chopp" },
      { name: "description", content: "Revenda chopp artesanal Golden Chopp com alta margem, suporte comercial e logística. Baixo investimento, produto premium e crescimento garantido." },
      { property: "og:title", content: "Programa de Revendedores Golden Chopp" },
      { property: "og:description", content: "Comece sua renda extra revendendo chopp artesanal premium." },
    ...
    <>
      <section className="section-pad">
        <div className="container-x text-center max-w-3xl mx-auto">
          ...
          <h1 className="text-4xl md:text-6xl font-display mt-3 mb-5">
            Comece sua <span className="text-gold-grad">renda extra</span> revendendo chopp premium.
          </h1>
          <p className="text-muted-foreground text-lg leading-relaxed">
            Produto consolidado, margem atrativa, suporte comercial completo e logística pronta.
            Faça parte da rede Golden Chopp.
          </p>
        </div>
      </section>
      ...
      <section className="section-pad bg-[var(--gradient-dark)]">
        <div className="container-x grid lg:grid-cols-2 gap-12 items-start">
          <div>
            <SectionTitle center={false} eyebrow="Como funciona" title="Simples, |direto| e lucrativo" />
            <ol className="space-y-5">
              {[
                ...
                <li key={t} className="flex gap-4">
                  <div className="w-9 h-9 rounded-full bg-[var(--gradient-gold)] text-primary-foreground grid place-items-center font-semibold shrink-0">{i + 1}</div>
                  <div>
...
function FormRevenda() {
  ...
  return (
    <form onSubmit={send} className="card-premium p-7 space-y-4">
      ...
      <Input label="Nome" value={f.nome} onChange={(v) => setF({ ...f, nome: v })} required maxLength={80} />
      <Input label="Cidade" value={f.cidade} onChange={(v) => setF({ ...f, cidade: v })} required maxLength={60} />
      <Input label="Telefone / WhatsApp" value={f.telefone} onChange={(v) => setF({ ...f, telefone: v })} required maxLength={20} />
      <div>
        <label className="text-xs uppercase tracking-wider text-muted-foreground">Tipo de negócio</label>
        <select value={f.tipo} onChange={(e) => setF({ ...f, tipo: e.target.value })}
          className="mt-1 w-full bg-secondary/60 border border-border rounded-lg px-3 py-2.5">
          {["Bar / Pub", "Restaurante", "Distribuidora", "Eventos", "Outro"].map((t) => <option key={t}>{t}</option>)}
        </select>
      </div>
...
function Input({ label, value, onChange, required, maxLength }: { label: string; value: string; onChange: (v: string) => void; required?: boolean; maxLength?: number }) {
  return (
    <div>
      <label className="text-xs uppercase tracking-wider text-muted-foreground">{label}</label>
      <input value={value} onChange={(e) => onChange(e.target.value)} required={required} maxLength={maxLength}
